...
const { throwMessage, classify } = require("../utils");
...
module.exports = class extends Generator {
  ...
  async writing() {
    ...
    switch (template) {
      case "angular11-lib": {
        const moduleName = classify(name); // UpperCamelCase
        const templateData = { name, moduleName };
        // 拷贝除projects目录
        this.fs.copyTpl(
          this.templatePath("**"),
        ...
        await Promise.allSettled(
          [
            ...
            this.fs.copyTplAsync(
              this.templatePath(`projects/my-lib/src/lib/my-lib.${file}`),
              this.destinationPath(`projects/${name}/src/lib/${name}.${file}`),
              templateData
            )
          )
        );
